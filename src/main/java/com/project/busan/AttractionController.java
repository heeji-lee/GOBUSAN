...
public class AttractionController {
	...
	@PreAuthorize("isAuthenticated()")
	public String insert(Attraction attraction, HttpServletRequest request) throws IOException {
		String saveDir = request.getSession().getServletContext().getRealPath("/");
		saveDir += "resources\\upload\\";
		MultipartFile f = attraction.getUploadFile();
		String fileName = "";
		if(!f.isEmpty()) {
	...
	public void findAll(String field, String word, String strPageNum, Model model) {
		...
		int totalCount = aservice.getCount(hm);
		int pageSize = 10;
		int pageBlock = 5;
		...
		List<Attraction> list = aservice.findAll(hm);
	...
	public String view(int num, Model model) {
		aservice.hitCount(num);
		Attraction attraction = aservice.findByNum(num);
		model.addAttribute("attraction", attraction);
		return "attraction/admin_view";
	}
	...
	public String update(Model model, int num) {
		Attraction attraction = aservice.findByNum(num);
		model.addAttribute("attraction", attraction);
		return "attraction/admin_update";
	}
	
	@PostMapping("admin_update")
	public String update(Attraction attraction) {
		aservice.update(attraction);
		return "redirect: admin_list";
	}
